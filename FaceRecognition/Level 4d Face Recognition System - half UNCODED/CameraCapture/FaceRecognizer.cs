...
{
    public partial class FaceRecogger : Form
    {
        ...
        private EigenFaceRecognizer recog;
        ...
        private void LoadTrainingSet()
        {
            ...
            int i = 0;
            ints = new int[Directory.EnumerateFiles("trainingset/", "*.bmp").Count()];
            imgs = new List<Image<Gray, byte>>();
            foreach (string file in Directory.EnumerateFiles("trainingset/", "*.bmp"))
            {
                Image InputImg = Image.FromFile(file);
                TestImage = new Image<Bgr, byte>(new Bitmap(InputImg));
                imgs.Add(TestImage.Convert<Gray,byte>());
                ints[i] = i;
                i++;
            ...
            MessageBox.Show("I loaded everything master");
